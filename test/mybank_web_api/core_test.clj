(ns mybank-web-api.core-test
  (:require
   ...
   [mybank-web-api.server :refer [server]]
...
(deftest api-test
  (let [_ (m.components/main)
    ...
    (testing "Make deposits"
      ...
      (is (= account-not-found-payload (m.utils/test-post server :post "/deposito/5" "{\"saldo\": 200}"))))
    (testing "Make withdraw"
      (is (= (assoc balance-account-1-payload :body "{\"saldo\": 200.0}")
             (m.utils/test-post server :post "/saque/1" "{\"value\": 100}")))
      (is (= account-not-found-payload
             (m.utils/test-post server :post "/saque/5" "{\"value\": 300.0}"))))
    (testing "Make withdraw without balance"
      (is (= account-without-balance-payload
             (m.utils/test-post server :post "/saque/1" "{\"value\": 200.1}"))))))
